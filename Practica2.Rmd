...
# Histograma de frecuencia de enlaces por tipo de URL
g1 <- ggplot(web_mediawiki, aes(x = as.numeric(veces_visto), fill = tipo_url)) +
  geom_histogram(bins = 10, alpha = 0.6, color = "black") +
  labs(title = "Frecuencia de aparición de enlaces",
       x = "Número de veces que aparece el enlace",
       y = "Frecuencia",
       fill = "Tipo de URL") +
  theme_minimal()

# Gráfico de barras: MediaWiki vs otros dominios
g2 <- ggplot(web_mediawiki, aes(x = destino, fill = destino)) +
  geom_bar(na.rm = TRUE) +
  labs(title = "Enlaces a MediaWiki vs otros dominios",
       x = "Destino del enlace",
       y = "Número de enlaces",
       fill = "Destino") +
  theme_minimal()

# Pie chart: porcentaje de status HTTP
...
# Componer todos los gráficos en una sola figura
(g1 / g2) | g3
