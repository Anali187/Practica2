...
**IMPORTANTE: a nivel de diseño hemos decidio obtener los enlaces existentes sólo en le BODY, NO en la cabezera de la página web**
...
# Mostrar la tabla con más de las seis primeras filas 
datatable(
  web_mediawiki,
  options = list(
...
```
```{r get_ahref_option2, include=FALSE}
url <- "https://www.mediawiki.org/wiki/MediaWiki"
page <- read_html(url)
...
# Obtencion texto y url de cada uno
links <- data.frame(
  text = link_text,
  url  = link_urls,
  stringsAsFactors = FALSE
...
# forzar el valor base del número de filas por página a 10
datatable(links,
          options = list(
            pageLength = 10  
          ))
...
```{r create_table_option2, include=TRUE}
...
datatable(
  web_mediawiki,
  colnames = c("Texto del enlace", "URL del enlace", "Veces visto"),
  options = list(
    pageLength = 50,   # Número de filas por página
    lengthMenu = c(5, 10, 20, 50),
    scrollY = "400px",
...
```{r create_table_just_6_rows, include=FALSE}
# Crear la columna 'veces_visto' sin dplyr
  web_mediawiki$veces_visto <- ave(web_mediawiki$url_enlace, web_mediawiki$url_enlace, FUN=length)
...
# 1. Asegurarse de que 'veces_visto' es numérico
  web_mediawiki$veces_visto <- as.numeric(web_mediawiki$veces_visto)

# 2. Gráfico de barras: frecuencia total de aparición por tipo de URL
...
g1 <- ggplot(df_frecuencia, aes(x = tipo_url, y = total_veces, fill = tipo_url)) +
  geom_col() +
  labs(
    title = "Frecuencia total de aparición de enlaces",
    x = "Tipo de URL",
    y = "Total apariciones",
    fill = "Tipo de URL"
  ) +
...
# 3. Gráfico de barras: MediaWiki vs otros dominios
  web_mediawiki$destino <- ifelse(
  !is.na(web_mediawiki$url_normalizada) &
    grepl("^http", web_mediawiki$url_normalizada) &
    !grepl("www.mediawiki.org", web_mediawiki$url_normalizada),
  "Otros dominios",
  "MediaWiki"
)
g2 <- ggplot(web_mediawiki, aes(x = destino, fill = destino)) +
  geom_bar(na.rm = TRUE) +
  labs(
    title = "Enlaces a MediaWiki vs otros dominios",
    x = "Destino",
...
# 4. Pie chart: porcentaje de status HTTP
  df_status <- web_mediawiki %>%
  group_by(Estado) %>%
  summarise(count = n(), .groups = "drop") %>%
  mutate(pct = count / sum(count) * 100)

g3 <- ggplot(df_status, aes(x = "", y = pct, fill = Estado)) +
...
# 4. Combinar los gráficos
# Patchwork: g1 y g2 arriba a la izquierda, pie chart a la derecha
(g1 / g2) | g3 +plot_layout(widths = c(1000,3))   # dos columnas: izquierda más ancha
